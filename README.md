...
| POST  | `/api/v1/users/resend`     | - Reenvía el código de verificación a un correo, el usuario todavía no verifica su registro.<br>- Si no encuentra el usuario retornar un 404.|
| POST  | `/api/v1/users/request-password-renewal`| - El usuario solo recuerda su correo y necesita generar otra contraseña, el usuario previamente ya se encuentra registrado.<br>- Si no encuentra el usuario retornar un 404.|
| POST  | `/api/v1/users/renew-password` | - El cliente indica el token y la nueva contraseña para su renovación, el usuario previamente ya se encuentra registrado.<br>- Si el token es inválido retornar un 400.|
| POST  | `/api/v1/users/change-password`| - Ocurre el cambio de contraseña, el usuario está logueado. <br>- Si el cambio es exitoso retornar un 200.<br>- Si las credenciales son inválidas retorna un 401.<br>- Solo usuarios autenticados pueden acceder a este endpoint. <br>- Si los datos son incorrectos retorna un código HTTP 400.|
